...
use \Exception;
use \PDO;
use ClanApp\entities\RolePermission as ChildRolePermission;
use ClanApp\entities\RolePermissionQuery as ChildRolePermissionQuery;
use ClanApp\entities\Map\RolePermissionTableMap;
 ...
 *
 * @method     \ClanApp\entities\PermissionQuery|\ClanApp\entities\RoleQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 *
 * @method     ChildRolePermission findOneById(int $id) Return the first ChildRolePermission filtered by the id column
 ...
 *
 * @method     ChildRolePermission requireOneById(int $id) Return the first ChildRolePermission filtered by the id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildRolePermission[]|ObjectCollection findById(int $id) Return ChildRolePermission objects filtered by the id column
...
abstract class RolePermissionQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \ClanApp\entities\Base\RolePermissionQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'FusionCore', $modelName = '\\ClanApp\\entities\\RolePermission', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, permission_id, role_id FROM role_permissions WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query by a related \ClanApp\entities\Permission object
     *
     * @param \ClanApp\entities\Permission|ObjectCollection $permission The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($permission instanceof \ClanApp\entities\Permission) {
            return $this
            ...
            throw new PropelException('filterByPermission() only accepts arguments of type \ClanApp\entities\Permission or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \ClanApp\entities\PermissionQuery A secondary query class using the current class as primary query
     */
    public function usePermissionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinPermission($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Permission', '\ClanApp\entities\PermissionQuery');
    }

    /**
     * Filter the query by a related \ClanApp\entities\Role object
     *
     * @param \ClanApp\entities\Role|ObjectCollection $role The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($role instanceof \ClanApp\entities\Role) {
            return $this
            ...
            throw new PropelException('filterByRole() only accepts arguments of type \ClanApp\entities\Role or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \ClanApp\entities\RoleQuery A secondary query class using the current class as primary query
     */
    public function useRoleQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinRole($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Role', '\ClanApp\entities\RoleQuery');
    }
    ...
    /**
     * Deletes all rows from the role_permissions table.
     *
