...
use \Exception;
use \PDO;
use ClanApp\entities\Role as ChildRole;
use ClanApp\entities\RoleQuery as ChildRoleQuery;
use ClanApp\entities\Map\RoleTableMap;
 ...
 *
 * @method     \ClanApp\entities\UserRoleQuery|\ClanApp\entities\RolePermissionQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class RoleQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \ClanApp\entities\Base\RoleQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'FusionCore', $modelName = '\\ClanApp\\entities\\Role', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name FROM roles WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query by a related \ClanApp\entities\UserRole object
     *
     * @param \ClanApp\entities\UserRole|ObjectCollection $userRole the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    {
        if ($userRole instanceof \ClanApp\entities\UserRole) {
            return $this
            ...
            throw new PropelException('filterByUserRole() only accepts arguments of type \ClanApp\entities\UserRole or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \ClanApp\entities\UserRoleQuery A secondary query class using the current class as primary query
     */
    public function useUserRoleQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinUserRole($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'UserRole', '\ClanApp\entities\UserRoleQuery');
    }
