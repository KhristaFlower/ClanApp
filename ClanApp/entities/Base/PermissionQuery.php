...
use \Exception;
use \PDO;
use ClanApp\entities\Permission as ChildPermission;
use ClanApp\entities\PermissionQuery as ChildPermissionQuery;
use ClanApp\entities\Map\PermissionTableMap;
 ...
 * @method     ChildPermissionQuery orderByTag($order = Criteria::ASC) Order by the tag column
 ...
 * @method     ChildPermissionQuery groupByTag() Group by the tag column
 ...
 *
 * @method     \ClanApp\entities\RolePermissionQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildPermission findOneByTag(string $tag) Return the first ChildPermission filtered by the tag column
 ...
 * @method     ChildPermission requireOneByTag(string $tag) Return the first ChildPermission filtered by the tag column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildPermission[]|ObjectCollection findByTag(string $tag) Return ChildPermission objects filtered by the tag column
...
abstract class PermissionQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \ClanApp\entities\Base\PermissionQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'FusionCore', $modelName = '\\ClanApp\\entities\\Permission', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, tag, name FROM permissions WHERE id = :p0';
        try {
    ...
    public function filterByTag($tag = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(PermissionTableMap::COL_TAG, $tag, $comparison);
    }
    ...
    /**
     * Filter the query by a related \ClanApp\entities\RolePermission object
     *
     * @param \ClanApp\entities\RolePermission|ObjectCollection $rolePermission the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    {
        if ($rolePermission instanceof \ClanApp\entities\RolePermission) {
            return $this
            ...
            throw new PropelException('filterByRolePermission() only accepts arguments of type \ClanApp\entities\RolePermission or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \ClanApp\entities\RolePermissionQuery A secondary query class using the current class as primary query
     */
    public function useRolePermissionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'RolePermission', '\ClanApp\entities\RolePermissionQuery');
    }
