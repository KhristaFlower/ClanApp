...
abstract class User implements ActiveRecordInterface
{
    ...
    /**
     * The value for the joined_clan_on field.
     * @var        \DateTime
     */
    protected $joined_clan_on;

    /**
     * The value for the left_clan_on field.
     * @var        \DateTime
     */
    protected $left_clan_on;
    ...
    /**
     * Get the [optionally formatted] temporal [joined_clan_on] column value.
     *
     *
     ...
     */
    public function getJoinedClanOn($format = NULL)
    {
        if ($format === null) {
            return $this->joined_clan_on;
        } else {
            return $this->joined_clan_on instanceof \DateTime ? $this->joined_clan_on->format($format) : null;
        }
    }

    /**
     * Get the [optionally formatted] temporal [left_clan_on] column value.
     *
     *
     * @param      string $format The date/time format string (either date()-style or strftime()-style).
     *                            If format is NULL, then the raw DateTime object will be returned.
     ...
     * @throws PropelException - if unable to parse/validate the date/time value.
     */
    public function getLeftClanOn($format = NULL)
    {
        if ($format === null) {
            return $this->left_clan_on;
        } else {
            return $this->left_clan_on instanceof \DateTime ? $this->left_clan_on->format($format) : null;
        }
    }
    ...
    public function setGameName($v)
    {
        ...
        if ($this->game_name !== $v) {
            $this->game_name = $v;
            $this->modifiedColumns[UserTableMap::COL_GAME_NAME] = true;
        }
    ...
    /**
     * Sets the value of [joined_clan_on] column to a normalized version of the date/time value specified.
     *
     * @param  mixed $v string, integer (timestamp), or \DateTime value.
     *               Empty strings are treated as NULL.
     * @return $this|\ClanApp\entities\User The current object (for fluent API support)
     */
    public function setJoinedClanOn($v)
    {
        $dt = PropelDateTime::newInstance($v, null, 'DateTime');
        if ($this->joined_clan_on !== null || $dt !== null) {
            if ($this->joined_clan_on === null || $dt === null || $dt->format("Y-m-d") !== $this->joined_clan_on->format("Y-m-d")) {
                $this->joined_clan_on = $dt === null ? null : clone $dt;
                $this->modifiedColumns[UserTableMap::COL_JOINED_CLAN_ON] = true;
            }
    ...
    /**
     * Sets the value of [left_clan_on] column to a normalized version of the date/time value specified.
     *
     * @param  mixed $v string, integer (timestamp), or \DateTime value.
     *               Empty strings are treated as NULL.
     * @return $this|\ClanApp\entities\User The current object (for fluent API support)
     */
    public function setLeftClanOn($v)
    {
        $dt = PropelDateTime::newInstance($v, null, 'DateTime');
        if ($this->left_clan_on !== null || $dt !== null) {
            if ($this->left_clan_on === null || $dt === null || $dt->format("Y-m-d") !== $this->left_clan_on->format("Y-m-d")) {
                $this->left_clan_on = $dt === null ? null : clone $dt;
                $this->modifiedColumns[UserTableMap::COL_LEFT_CLAN_ON] = true;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UserTableMap::translateFieldName('GameName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->game_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UserTableMap::translateFieldName('JoinedClanOn', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            $this->joined_clan_on = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : UserTableMap::translateFieldName('LeftClanOn', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            $this->left_clan_on = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;
            $this->resetModified();
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_GAME_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'game_name';
        }
        if ($this->isColumnModified(UserTableMap::COL_JOINED_CLAN_ON)) {
            $modifiedColumns[':p' . $index++]  = 'joined_clan_on';
        }
        if ($this->isColumnModified(UserTableMap::COL_LEFT_CLAN_ON)) {
            $modifiedColumns[':p' . $index++]  = 'left_clan_on';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'game_name':
                        ...
                        break;
                    case 'joined_clan_on':
                        $stmt->bindValue($identifier, $this->joined_clan_on ? $this->joined_clan_on->format("Y-m-d H:i:s") : null, PDO::PARAM_STR);
                        break;
                    case 'left_clan_on':
                        $stmt->bindValue($identifier, $this->left_clan_on ? $this->left_clan_on->format("Y-m-d H:i:s") : null, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function getByPosition($pos)
    {
        switch ($pos) {
            ...
            case 4:
                ...
                break;
            case 5:
                return $this->getJoinedClanOn();
                break;
            case 6:
                return $this->getLeftClanOn();
                break;
            default:
                return null;
                break;
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[4] => $this->getGameName(),
            $keys[5] => $this->getJoinedClanOn(),
            $keys[6] => $this->getLeftClanOn(),
        );
    ...
    public function setByPosition($pos, $value)
    {
        switch ($pos) {
            ...
            case 4:
                ...
                break;
            case 5:
                $this->setJoinedClanOn($value);
                break;
            case 6:
                $this->setLeftClanOn($value);
                break;
        } // switch()
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) {
            $this->setGameName($arr[$keys[4]]);
        }
        if (array_key_exists($keys[5], $arr)) {
            $this->setJoinedClanOn($arr[$keys[5]]);
        }
        if (array_key_exists($keys[6], $arr)) {
            $this->setLeftClanOn($arr[$keys[6]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_GAME_NAME)) {
            $criteria->add(UserTableMap::COL_GAME_NAME, $this->game_name);
        }
        if ($this->isColumnModified(UserTableMap::COL_JOINED_CLAN_ON)) {
            $criteria->add(UserTableMap::COL_JOINED_CLAN_ON, $this->joined_clan_on);
        }
        if ($this->isColumnModified(UserTableMap::COL_LEFT_CLAN_ON)) {
            $criteria->add(UserTableMap::COL_LEFT_CLAN_ON, $this->left_clan_on);
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setGameName($this->getGameName());
        $copyObj->setJoinedClanOn($this->getJoinedClanOn());
        $copyObj->setLeftClanOn($this->getLeftClanOn());
    ...
    public function clear()
    {
        ...
        $this->email = null;
        $this->game_name = null;
        $this->joined_clan_on = null;
        $this->left_clan_on = null;
        $this->alreadyInSave = false;
        $this->clearAllReferences();
        $this->resetModified();
        $this->setNew(true);
